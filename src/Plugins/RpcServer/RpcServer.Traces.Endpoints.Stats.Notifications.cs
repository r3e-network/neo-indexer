...
//
// RpcServer.Traces.Endpoints.Stats.Notifications.cs file belongs to the neo project and is free
...
namespace Neo.Plugins.RpcServer
{
    ...
    {
        [RpcMethod]
        protected internal virtual async Task<JToken> GetNotificationStats(JArray _params)
        {
            NotificationStatsQueryOptions options;
            if (_params.Count >= 2 && _params[0] is not JObject)
            {
                options = new NotificationStatsQueryOptions
                {
                ...
                if (_params.Count > 2)
                {
                    var extra = ParseNotificationStatsOptions(_params[2]);
            ...
            {
                options = ParseNotificationStatsOptions(_params.Count > 0 ? _params[0] : null);
            }
            ...
            var results = await SendSupabaseRpcAsync<NotificationStatsResult>(settings, "get_notification_stats", payload).ConfigureAwait(false);
            ...
            if (!string.IsNullOrEmpty(options.EventName))
                json["eventName"] = options.EventName;
