...
//
// RpcServer.Traces.Endpoints.Stats.StorageReads.cs file belongs to the neo project and is free
...
namespace Neo.Plugins.RpcServer
{
    ...
    {
        [RpcMethod]
        protected internal virtual async Task<JToken> GetStorageReadStats(JArray _params)
        {
            StorageReadStatsQueryOptions options;
            if (_params.Count >= 2 && _params[0] is not JObject)
            {
                options = new StorageReadStatsQueryOptions
                {
                ...
                if (_params.Count > 2)
                {
                    var extra = ParseStorageReadStatsOptions(_params[2]);
            ...
            {
                options = ParseStorageReadStatsOptions(_params.Count > 0 ? _params[0] : null);
            }
            ...
            var results = await SendSupabaseRpcAsync<StorageReadStatsResult>(settings, "get_storage_read_stats", payload).ConfigureAwait(false);
