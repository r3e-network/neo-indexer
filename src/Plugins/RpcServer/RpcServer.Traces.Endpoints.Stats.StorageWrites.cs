...
//
// RpcServer.Traces.Endpoints.Stats.StorageWrites.cs file belongs to the neo project and is free
...
namespace Neo.Plugins.RpcServer
{
    ...
    {
        [RpcMethod]
        protected internal virtual async Task<JToken> GetStorageWriteStats(JArray _params)
        {
            StorageWriteStatsQueryOptions options;
            if (_params.Count >= 2 && _params[0] is not JObject)
            {
                options = new StorageWriteStatsQueryOptions
                {
                ...
                if (_params.Count > 2)
                {
                    var extra = ParseStorageWriteStatsOptions(_params[2]);
            ...
            {
                options = ParseStorageWriteStatsOptions(_params.Count > 0 ? _params[0] : null);
            }
            ...
            var results = await SendSupabaseRpcAsync<StorageWriteStatsResult>(settings, "get_storage_write_stats", payload).ConfigureAwait(false);
