...
function BlockStatePage() {
  ...
  const handleSearch = useCallback(async (filters: BlockSearchFilters, pageNum = 1) => {
    ...
    try {
      // If searching by specific block index, fetch that block directly
                ...
                <div className="block-list">
                  {blocks.data.map((block) => (
                    <div
                      ...
                      role="button"
                      tabIndex={0}
                      onKeyDown={(e) => e.key === 'Enter' && handleBlockSelect(block)}
                    >
